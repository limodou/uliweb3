...
from uliweb.utils._compat import string_types, u, callable, import_, next, text_type, PY2
...
class ReferenceSelectField(SelectField):
    def __init__(self, model, group_field=None, value_field='id', condition=None, 
        ...
        self.query = query
        self.get_display = get_display or u
    ...
    def __set__(self, model_instance, value):
        ...
        if value:
            if isinstance(value, (tuple, list)):
                ...
                if isinstance(table_id, string_types):
...
def get_obj_url(obj):
    ...
    if hasattr(obj, 'get_url'):
        ...
        if url:
            display = str(Tag('a', text_type(obj), href=url.format(obj.to_dict())))
        else:
            display = text_type(obj)
    return display
...
#                display = prop.get_display_value(value)
    ...
    if PY2 and isinstance(display, text_type):
...
class AddView(object):
    ...
    def get_url(self, obj):
        ...
        if callable(self.ok_url):
            args = inspect.signature(self.ok_url).parameters
            kwargs = {}
            _dict = obj.to_dict()
            for k in args:
                if k in _dict:
...
class DeleteView(object):
    ...
    def delete_manytomany(self, obj):
        for k, v in obj._manytomany.items():
            getattr(obj, k).clear()
